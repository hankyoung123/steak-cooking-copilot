...
| Hero flip | First `flipNow` event | Make the required flip unmistakable | MotionDirector + full-bleed photo layer | 550 ms | Re-triggerable | Crossfade + FLIP NOW | Low: transform-only keyframes |
| Compact flip | Later `flipNow` event | Reinforce action without fatigue | MotionDirector + full-bleed photo layer | 380 ms | Re-triggerable | Label + haptic | Low: rotation/scale only |
...
| Take out | Engine emits `pullNow`; session changes to resting from user action | Communicate urgency and end active cooking | Domain state + MotionDirector + full-bleed photo layer | 620 ms | Yes | Crossfade to the rest photo | Low: transition transforms |
...
| Ready | Engine completes resting | Deliver a distinct completion moment | Domain state + MotionDirector + result reveal | 950 ms | Yes | Opacity + scale ≤1.02 | Low: transform/crossfade only |
